...
interface KegiatanSelesai {
  ...
  completedAt: Date;
  completedBy: string;
  feedback?: FeedbackItem[]; // Ubah menjadi array
  createdBy: string;
}

// Fungsi untuk download Excel
const downloadFeedbackExcel = (kegiatanList: KegiatanSelesai[]) => {
  // Siapkan data untuk Excel dari semua feedback
  const excelData: any[] = [];
...
export default function KegiatanSelesaiPage() {
      ...
      const unsubscribe = onSnapshot(q, 
        (snapshot) => {
          const kegiatanData: KegiatanSelesai[] = snapshot.docs.map(doc => {
            ...
            // Konversi feedback ke array jika masih object lama
            ...
            if (data.feedback) {
              if (Array.isArray(data.feedback)) {
                ...
                // Jika masih object (format lama), konversi ke array
                feedbackArray = [{
  ...
  // Hitung jumlah total feedback dari semua kegiatan
